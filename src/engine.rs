...
impl Engine {
    ...
    pub async fn delete(&mut self, key: &[u8]) -> EngineResult<()> {
        ...
        let record = WALRecord::new(key.to_vec(), None, timestamp, sequence_number);
        self.wal.write_record(&record)?;
    ...
    /// Recover database state from WAL files
    fn recover_from_wal(&mut self) -> EngineResult<()> {
        info!("Starting WAL recovery");

        // Use the existing WAL to recover data
        self.wal.recover(&self.memtable)?;

        // Update sequence number from WAL
        {
            let mut seq = self.sequence_number.write().unwrap();
